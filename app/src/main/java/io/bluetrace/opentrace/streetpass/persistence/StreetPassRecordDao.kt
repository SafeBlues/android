...
interface StreetPassRecordDao {
    ...
    @Query("UPDATE record_table SET processed = 1 WHERE tempId = :tempId")
    ...
    @Query("SELECT * FROM record_table WHERE tempId = :tempId AND NOT processed")
    suspend fun getAllUnprocessedRecordsForTempId(tempId: String): List<StreetPassRecord>
